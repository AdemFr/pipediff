...
def test_log_empty_frame(tracker: PipeTracker) -> None:
    result = tracker.log_frame(pd.DataFrame(), return_result=True)
    assert result == FrameLog()


def test_reset_tracker_is_empty(tracker: PipeTracker) -> None:
    tracker.log_frame(pd.DataFrame())
    tracker.reset()
    assert len(tracker.logs) == 0


def test_log_multiple_frames(tracker: PipeTracker) -> None:
    tracker.log_frame(pd.DataFrame())
    tracker.log_frame(pd.DataFrame())
...
def test_frame_log_access(tracker: PipeTracker, df_num: pd.DataFrame) -> None:
...
def test_log_frame_with_name(tracker: PipeTracker) -> None:
...
def test_agg_method_format_options_yield_same_result(tracker: PipeTracker, df_num: pd.DataFrame) -> None:
...
def test_nans_and_agg_for_both_axis(tracker: PipeTracker, df_num: pd.DataFrame) -> None:
...
def test_full_and_sliced_copy(tracker: PipeTracker, df_all_types: pd.DataFrame) -> None:
...
def test_log_dtypes(tracker: PipeTracker, df_all_types: pd.DataFrame) -> None:
    result = tracker.log_frame(df_all_types, dtypes=True, return_result=True)
    assert result.dtypes == dict(df_all_types.dtypes)


def test_log_shapes(tracker: PipeTracker, df_all_types: pd.DataFrame) -> None:
...
def test_log_colums(tracker: PipeTracker, df_all_types: pd.DataFrame) -> None:
