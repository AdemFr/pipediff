...
from pipelog.frame_log import FrameLog, _NEW_LOG_KEY


def test_default_attributes(tracker: PipeLogger) -> None:
    assert len(tracker.logs) == 0


def test_log_empty_frame(tracker: PipeLogger) -> None:
    result = tracker.log_frame(pd.DataFrame(), return_result=True)
    assert result == FrameLog()


def test_reset_tracker_is_empty(tracker: PipeLogger) -> None:
    tracker.log_frame(pd.DataFrame())
    tracker.reset()
    assert len(tracker.logs) == 0


def test_log_multiple_frames(tracker: PipeLogger) -> None:
    tracker.log_frame(pd.DataFrame())
    tracker.log_frame(pd.DataFrame())
...
def test_frame_log_access(tracker: PipeLogger, df_num: pd.DataFrame) -> None:
...
def test_log_frame_with_name(tracker: PipeLogger) -> None:
...
def test_agg_method_format_options_yield_same_result(tracker: PipeLogger, df_num: pd.DataFrame) -> None:
...
def test_nans_and_agg_for_both_axis(tracker: PipeLogger, df_num: pd.DataFrame) -> None:
...
def test_full_and_sliced_copy(tracker: PipeLogger, df_all_types: pd.DataFrame) -> None:
...
def test_log_dtypes(tracker: PipeLogger, df_all_types: pd.DataFrame) -> None:
    result = tracker.log_frame(df_all_types, dtypes=True, return_result=True)
    assert result.dtypes == dict(df_all_types.dtypes)


def test_log_shapes(tracker: PipeLogger, df_all_types: pd.DataFrame) -> None:
...
def test_log_colums(tracker: PipeLogger, df_all_types: pd.DataFrame) -> None:
